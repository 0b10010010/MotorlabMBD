classdef Encoder < realtime.internal.SourceSampleTime ... % Inherits from matlab.System
    ...
    methods (Access=protected)
        function setupImpl(obj) %#ok<MANU>
            if coder.target('Rtw')
                ...
                coder.cinclude('Encoder.h');
                coder.ceval('MotorEncoderInit');
%                 coder.ceval('LoadEncoderInit');
                coder.ceval('set', 0.0);
            else
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, context)
            if context.isCodeGenTarget('rtw')
                ...
                % linker flags
                addIncludeFiles(buildInfo,'Encoder.h');         
...
%                 addSourceFiles(buildInfo,'stm32f4xx_tim.c', srcDir);
                addSourceFiles(buildInfo,'Encoder.c',srcDir);
